...
class StepConfig(BaseModel):
    ...
    strategy_type: str = Field(..., description="Type of step strategy to use")
...
class GraphConfig(BaseModel):
    ...
    graph_type: str = Field(default="linear", description="Type of graph topology")
...
class AgentConfig(BaseModel):
    ...
    def validate_config(self) -> "AgentConfig":
        """Validate configuration integrity after model creation."""
        # Basic validation - can be extended as needed
        if not self.steps:
            # For now, allow empty steps - this can be made configurable
            pass
        return self
