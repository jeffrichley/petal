...
@pytest.mark.asyncio
async def test_with_tools_string_names():
    """Test with_tools() using string names that get resolved via ToolFactory."""
    ...
    # Mock the tool factory to return our test tool
    with patch.object(ToolFactory, "resolve", return_value=test_tool):
        agent = AgentFactory(ChatState).with_chat().with_tools(["test_tool"]).build()
...
@pytest.mark.asyncio
async def test_with_tools_direct_objects():
    """Test with_tools() using direct tool objects."""
    from langchain_core.tools import tool
    ...
    agent = AgentFactory(ChatState).with_chat().with_tools([calculator]).build()
...
@pytest.mark.asyncio
async def test_with_tools_mixed_input():
    """Test with_tools() with mixed string names and direct objects."""
    from langchain_core.tools import tool
    ...
    def string_tool(query: str) -> str:
        """A tool that will be referenced by string."""
        return f"String: {query}"

    # Mock the tool factory to return our string tool
    with patch.object(ToolFactory, "resolve", return_value=string_tool):
        agent = (
            AgentFactory(ChatState)
            .with_chat()
            .with_tools(["string_tool", direct_tool])
            .build()
        )
